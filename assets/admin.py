...
class ExportCsvMixin:
    def export_as_csv(self, request, queryset):
        ...
        field_names = [field.name for field in meta.fields if not field.auto_created ] # ADD INDEX_COLUMN
        ...
        for obj in queryset:
            row = writer.writerow(
                [smart_str(getattr(obj, field)) for field in field_names]
            )
    ...
    def import_csv(self, request):
        if request.method == "POST":
            ...
            try:
                ...
                if created[1] == True:
                    messages.success(
                        request, "Sucessfully created " + str(row_count) + " assets"
                    )
                else:
